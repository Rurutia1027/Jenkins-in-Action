...
This document is the frontend test design for Bug Tracker. It follows the lifecycle model in [testing-architecture.md](./testing-architecture.md) and [README.md](../README.md):

```
Test Type → Test Strategy → Suite → Engineering Scenario → Lifecycle / Environment → Pipeline
```

A **test type** describes **what** the UI proves (unit, component, E2E, visual).
A **test strategy** (smoke, regression, sanity) **selects** those types. It is not a new driver.
An **engineering scenario** (PR, nightly, release, canary) is **why the pipeline ran**.

Smoke, sanity, and regression in the README type list are treated here as **composite strategies**. They do not get a fifth frontend framework. 
...
- **Test suite**: A runnable package of one type, for example `bugtracker-frontend` Jest, `tests-e2e`, `tests-visual`.
- **Test scenario**: Engineering trigger: PR Validation, Master Validation, Nightly, Release, Performance, Production / Canary. 
...
| **Integration Testing**        | Not owned      | Platform suite `tests-integration` (handler → bbolt), not Jest-with-real-backend | Jest-with-real-backend is not an integration suite here; that path is E2E |
...
| `fe-bdd`       | BDD/Acceptance Testing     | `tests-bdd/` Gherkin + API steps | Master, Nightly, Release                       |
| `fe-e2e`       | E2E Testing                | `tests-e2e`                                     | Master, Nightly, Release                       |
| `fe-visual`    | Visual Regression          | `tests-visual` Playwright `toHaveScreenshot`    | Nightly, Release                               |
| `fe-smoke`     | Smoke Testing              | `tests-e2e/smoke.spec.ts`                       | After deploy, Production Smoke                 |
| `fe-sanity`    | Sanity Testing             | Selected `fe-component` and/or `fe-e2e`         | On-demand / change-scoped                      |
| `fe-security`  | Security Testing           | `tests-security` (npm audit + frontend image) | PR (audit), Master / Nightly / Release (image) |
...
| Shared API fixture module                    | Exists in `tests/fixtures/`                                   |
| Visual checkpoints                           | Exists in `tests-visual/` (Playwright `toHaveScreenshot`)     |
| `fe-smoke` slice                             | Exists in `tests-e2e/smoke.spec.ts`; not wired to Release yet |
| BDD on the four journeys                     | Exists in `tests-bdd/`                                        |
| Profile wiring in Jenkins                    | Partial: security + BDD on PR/Master/Nightly/Release as designed; Nightly runs E2E + visual when stable is deployed |


When implementing remaining suites, reuse `tests/fixtures`. Do not start by wrapping the current E2E file in Gherkin. See [test-implementation-gaps.md](./test-implementation-gaps.md). 
